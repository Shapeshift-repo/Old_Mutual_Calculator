...
export default function Banner({ heading = null, subHeading = null, content = null, image = null, icon = null, link = null, linkIcon = null, linkIconClasses = "", className = '' }){
  return (
    <div className={`relative h-[925px] w-full rounded-bl-[214px] rounded-br-[214px] lg:rounded-bl-[317px] lg:rounded-br-[317px] overflow-hidden ${className}`}>
      ...
      <div 
        className="banner-content absolute bottom-0 w-full h-[100%] pt-[390px] px-[45px] lg:px-[80px] pb-[30px] text-center"
        style={{ background: 'linear-gradient(8.46deg, rgba(0, 150, 119, 0.93) 17.02%, rgba(80, 184, 72, 0.93) 44.72%, rgba(80, 184, 72, 0) 78.61%)' }}
      >

        {icon && (
            <div className="icon-holder flex justify-center mb-[36px]">
                {icon}
            </div>
        )}
        ...
        {link && (
          <div className="flex justify-center mt-[36px] relative z-10">
            <Link href={link} className={`link transition-transform duration-200 ease-in-out ${linkIconClasses}`}>         
                {linkIcon || `    
